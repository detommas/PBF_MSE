...
pwin = "C:\\Users\\desiree.tommasi\\Documents\\Bluefin\\Github\\PBF_MSE-main2\\PBF_MSE\\"
...
for (tstep in 1:length(asmt_t)){
  ...
  if (sa==0) {
    ...
    TAC_mat = HCR8_pbf_byfleet_f_25for(ssout=om_out, dat = SPRmat, forf=ben, yr=yr_end, SSBtrs=ssb_thr, SSBlim=40724.6, err=1, Cmin = cmin[,3:5], hs=hs,hcr=hcr,scn=scn,itr=itr,tstep=tstep,yrb=c(2002:2004),yrf=yfor,tacl=tacl)
    ...
    #Generate TAC based on current harvest control rule
    TAC_mat = HCR8_pbf_byfleet_f_25for(ssout=em_out, dat = SPRmat, forf=ben, yr=yr_end, SSBtrs=ssb_thr, SSBlim=40724.6, err=1, Cmin = cmin[,3:5], hs=hs,hcr=hcr,scn=scn,itr=itr,tstep=tstep,yrb=c(2002:2004),yrf=yfor, tacl=tacl)
...
#****************************************************************************
#Step 6: Run the OM with estimation on the last time step with the data with no error
