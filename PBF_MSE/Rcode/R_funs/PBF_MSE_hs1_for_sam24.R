...
PBF_MSE_hs1_for_sam24 = function(hsnum,hcrnum,scnnum,itr, Bthr, Blim, sa, Fmin, lag, obse, aspm, yfor,tacl) { 
...
for (tstep in 1:length(asmt_t)){
  ...
  #Record the TAC - set in the previous time step but effective for the current, recorded for the claendar year
  ...
  TACWmdt[(asmt_t[tstep]-1):(asmt_t[tstep]+(tasmt-2))] = rep(TACWm,tasmt)
  ...
  if (sa==0) {
    ...
    if (tacl>0){
      TAC_mat = HCR1a_pbf_byfleet_f_25for(ssout=om_out, dat = SPRmat, yr=yr_end, SSBtrs=ssb_thr, SSBlim=ssb_lim, err=1, Fmin=Fmin,hs=hs,hcr=hcr,scn=scn,itr=itr,tstep=tstep,yrb=c(2002:2004),yrf=yfor,tacl=tacl)
    } else{
      TAC_mat = HCR1a_pbf_byfleet_f(ssout=om_out, dat = SPRmat, yr=yr_end, SSBtrs=ssb_thr, SSBlim=ssb_lim, err=1, Fmin=Fmin,hs=hs,hcr=hcr,scn=scn,itr=itr,tstep=tstep,yrb=c(2002:2004),yrf=yfor)
    }
    ...
    if (lag==0) {
      ...
      Ftgt_em [(asmt_t[tstep]-ryr):asmt_t[tstep]] = rep(om_ftgt,(ryr+1))
      ...
      if (tstep==1){
        ...
        Ftgt_em [(asmt_t[tstep]-1)] = om_ftgt
        ...
        Ftgt_em [(asmt_t[tstep]-tasmt):(asmt_t[tstep]-1)] = rep(om_ftgt,tasmt)
    ...
    #****************************************************************************
    #Step 3: Generate files for operating model - note that since em is being run lag is > 0
    OM_fun_tvry_adj_lag(pdir, sdir, hs, hcr, scn, hsw, hcrw, scnw, pwin, itr, tstep, tasmt, rec_devs, sdev, lag=lag,yfor=yfor)
    ...
    #specify years over which to compute biology (yrb) and exploitation pattern (yrf)
    if (tacl>0){
      TAC_mat = HCR1a_pbf_byfleet_f_25for(ssout=em_out, dat = SPRmat, yr=yr_end, SSBtrs=ssb_thr, SSBlim=ssb_lim, err=1, Fmin=Fmin,hs=hs,hcr=hcr,scn=scn,itr=itr,tstep=tstep,yrb=c(2002:2004),yrf=yfor,tacl=tacl)
    } else{
      TAC_mat = HCR1a_pbf_byfleet_f(ssout=em_out, dat = SPRmat, yr=yr_end, SSBtrs=ssb_thr, SSBlim=ssb_lim, err=1, Fmin=Fmin,hs=hs,hcr=hcr,scn=scn,itr=itr,tstep=tstep,yrb=c(2002:2004),yrf=yfor)
    }
    ...
    saveRDS(TAC_mat, file_tac)
      ...
      if (tstep==1){
        B0dat[(asmt_t[tstep]-1)] = SPRmat$SSBzero[1]
        Ftgt_em [(asmt_t[tstep]-1)] = em_ftgt
        R_em[(asmt_t[tstep]-1)] = SPRmat$Recruits[(dim(SPRmat)[1]-1)]
        SPB_em[(asmt_t[tstep]-1)] = SPRmat$SSB[(dim(SPRmat)[1]-1)]
        D_em[(asmt_t[tstep]-1)] = SPRmat$Deplete[(dim(SPRmat)[1]-1)]
        SPR_em[(asmt_t[tstep]-1)] = SPRmat$SPR[(dim(SPRmat)[1]-1)]
        ...
        B0dat[(asmt_t[tstep]-tasmt):(asmt_t[tstep]-1)] = SPRmat$SSBzero[1]
        Ftgt_em [(asmt_t[tstep]-tasmt):(asmt_t[tstep]-1)] = rep(em_ftgt,tasmt)
        R_em[(asmt_t[tstep]-tasmt):(asmt_t[tstep]-1)] = SPRmat$Recruits[(dim(SPRmat)[1]-tasmt):(dim(SPRmat)[1]-1)]
        SPB_em[(asmt_t[tstep]-tasmt):(asmt_t[tstep]-1)] = SPRmat$SSB[(dim(SPRmat)[1]-tasmt):(dim(SPRmat)[1]-1)]
        D_em[(asmt_t[tstep]-tasmt):(asmt_t[tstep]-1)] = SPRmat$Deplete[(dim(SPRmat)[1]-tasmt):(dim(SPRmat)[1]-1)]
        SPR_em[(asmt_t[tstep]-tasmt):(asmt_t[tstep]-1)] = SPRmat$SPR[(dim(SPRmat)[1]-tasmt):(dim(SPRmat)[1]-1)]
...
outmat1 = data.frame(Year = 2023:((length(asmt_t)*3)+2021), R = Rdat[1:23], Rem = R_em[1:23], SSB = SPBdat[1:23], SSBem = SPB_em[1:23], Depletion = Ddat[1:23], Dem = D_em[1:23], SPR = SPRdat[1:23], SPRem = SPR_em[1:23], Catch = Tdat[1:23], Cem = C_em[1:23], TAC = TACdt[1:23], TACws = TACWsdt[1:23],TACwl = TACWldt[1:23], TACwm = TACWmdt[1:23], TACepo = TACEdt[1:23], B0em = B0dat[1:23], B0om = B0dat_om[1:23], Bsmry= Btot[1:23], Bsmryem= Btot_em[1:23], Ftgtom = Ftgt_om[1:23], Ftgtem = Ftgt_em[1:23])
