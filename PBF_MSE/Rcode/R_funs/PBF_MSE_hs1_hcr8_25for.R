...
#Generate the minimum catch based on the current CMM
TACWsm = 4341 #4475 catch limit for small fish in western pacific reduced by 3% to 4341 to account for small fish caught by mixed 
TACWlm= 7624 #7860 catch limit for large fish in western pacific reduced by 3% to 7624 to account for large fish caught by mixed
TACWmm = 370
TACEm= 3995 #catch limit for EPO
TACdatm = TACWsm+TACWlm+TACEm+TACWmm
...
new_cdatwlm = cr_wl
new_cdatwlm$catch = cr_wl$cratio*TACWlm

new_cdatwsm = cr_ws
new_cdatwsm$catch = cr_ws$cratio*TACWsm

new_cdatwmm = cr_wm
new_cdatwmm$catch = cr_wm$cratio*TACWmm

new_cdatem = cr_epo
new_cdatem$catch = cr_epo$cratio*TACEm

cmin = rbind(new_cdatwlm,new_cdatwsm,new_cdatwmm,new_cdatem)
#**************************************************************************************
...
for (tstep in 1:length(asmt_t)){
  ...
  #*************************************************************************************
  if (tstep == 1) {
    TACWs = TACWsm #4475 catch limit for small fish in western pacific reduced by 3% to 4341 to account for small fish caught by mixed 
    TACWl= TACWlm #7860 catch limit for large fish in western pacific reduced by 3% to 7624 to account for large fish caught by mixed
    TACWm = TACWmm
    TACE= TACEm #catch limit for EPO
    TACdat = TACdatm
    
    #Generate catch by fleet and season using the Ws, Wl, epo catch ratios (cr_hcr) produced by the
    #catch_ratios.R code and the TAC
    new_cdatwl = new_cdatwlm
    new_cdatws = new_cdatwsm
    new_cdatwm = new_cdatwmm
    new_cdate = new_cdatem
    new_cdat = cmin
    ...
    TACWm = TACmat$TACWm #catch limits for mixed fleets in western pacific
  ...
  if (sa==0) {
    ...
    TAC_mat = HCR8_pbf_byfleet_f_25for(ssout=om_out, dat = SPRmat, yr=yr_end, SSBtrs=ssb_thr, SSBlim=40724.6, err=1, Cmin = cmin[,2:4], hs=hs,hcr=hcr,scn=scn,itr=itr,tstep=tstep,yrb=c(2002:2004),yrf=c(2017:2019))
    ...
    #Generate TAC based on current harvest control rule
    TAC_mat = HCR8_pbf_byfleet_f_25for(ssout=em_out, dat = SPRmat, yr=yr_end, SSBtrs=ssb_thr, SSBlim=40724.6, err=1, Cmin = cmin[,2:4],hs=hs,hcr=hcr,scn=scn,itr=itr,tstep=tstep,yrb=c(2002:2004),yrf=c(2017:2019))
