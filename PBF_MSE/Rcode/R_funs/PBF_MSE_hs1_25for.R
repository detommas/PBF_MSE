...
for (tstep in 1:length(asmt_t)){
  ...
  if (sa==0) {
    ...
    TAC_mat = HCR1a_pbf_byfleet_f_25for(ssout=om_out, dat = SPRmat, yr=yr_end, SSBtrs=ssb_thr, SSBlim=ssb_lim, err=1, Fmin=Fmin,hs=hs,hcr=hcr,scn=scn,itr=itr,tstep=tstep,yrb=c(2002:2004),yrf=c(2017:2019))
    ...
    #Generate TAC based on current harvest control rule
    TAC_mat = HCR1a_pbf_byfleet_f_25for(ssout=em_out, dat = SPRmat, yr=yr_end, SSBtrs=ssb_thr, SSBlim=ssb_lim, err=1, Fmin=Fmin,hs=hs,hcr=hcr,scn=scn,itr=itr,tstep=tstep,yrb=c(2002:2004),yrf=c(2017:2019))
