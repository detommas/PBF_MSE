...
PBF_MSE_hs2_910_for = function(hsnum,hcrnum,scnnum,itr, Bthr, sa, lag) { 
...
for (tstep in 1:length(asmt_t)){
  ...
  if (sa==0) {
    ...
    TAC_mat = HCR2_pbf_byfleet_f(ssout=om_out, dat = SPRmat, yr=yr_end, SSBtrs=ssb_trs, err=1, hs=hs,hcr=hcr,scn=scn,itr=itr,tstep=tstep,yrb=c(2002:2004),yrf=c(2017:2019))
    ...
    #Generate TAC based on current harvest control rule
    TAC_mat = HCR2_pbf_byfleet_f(ssout=em_out, dat = SPRmat, yr=yr_end, SSBtrs=ssb_trs, err=1, hs=hs,hcr=hcr,scn=scn,itr=itr,tstep=tstep,yrb=c(2002:2004),yrf=c(2017:2019))
