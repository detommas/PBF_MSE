...
HCR8_pbf_byfleet_f_25for2 <- function(ssout, dat, forf, yr, SSBtrs, SSBlim, Ftgt,cr, err, Cmin, hs,hcr,scn,itr,tstep, yrb,yrf,tacl,TACdt,TACEdt,TACWldt,TACWsdt, TACmat){
  ...
  #calculate catch ratio with which to split the medium fleet TAC by fleet and season
  ...
  #Potential TAC by fleet segment based on CMM 2021-02 and C-21-05
  ChcrWs = 4725
  ChcrWl = 7609
  ChcrE = 5472.761 #3995 (half of 2023-2024 biennial limit) plus rec catches of FY2022)
  ...
  #TAC table by fleet and season that will be fed back into OM
  TAC_tablei = Cmin #this is a table with the catch by fleet set by CMM 2021-02 and C-21-05 plus the catches for the rec and discard fleets
  
  names(TAC_tablei)[3] = "TAC"
  
  #calculate catch ratio with which to split the medium fleet TAC by fleet and season
  mcrmin=TAC_tablei$TAC[TAC_tablei$Fleet %in% c(11,17:19)]/sum(TAC_tablei$TAC[TAC_tablei$Fleet %in% c(11,17:19)])
  ...
  if (tacl>0){
    if (abs(CchangeWs) > (tacl/100) & CchangeWs > 0){
      ...
      TACWs = ChcrWs
      TAC_tablei2$TAC[TAC_tablei2$Fleet %in% c(8:10,12:16)] = TAC_tablei$TAC[TAC_tablei$Fleet %in% c(8:10,12:16)]
      #find leftover small TAC for medium sized fleet
      ms=TACWs-sum(TAC_tablei2$TAC[TAC_tablei2$Fleet %in% c(8:10,12:16)])
    }
    #do for Wl
    if (abs(CchangeWl) > (tacl/100) & CchangeWl > 0){
      ...
      TACWl = 7609
      TAC_tablei2$TAC[TAC_tablei2$Fleet %in% c(1:7)] = TAC_tablei$TAC[TAC_tablei$Fleet %in% c(1:7)]
      #find leftover large TAC for medium sized fleet
      ml=TACWl-sum(TAC_tablei2$TAC[TAC_tablei2$Fleet %in% c(1:7)])
    }
    
    #Apply the TAC to the mixed fleets
    TAC_tablei2$TAC[TAC_tablei2$Fleet %in% c(11,17:19)]=(ms+ml)*mcrmin
    ...
    if (abs(CchangeE) > (tacl/100) & CchangeE > 0){
      ...
      TACE = 5472.761
      TAC_tablei2$TAC[TAC_tablei2$Fleet %in% c(20:23)] = TAC_tablei$TAC[TAC_tablei$Fleet %in% c(20:23)]
    }
    ...
    TACE = 5472.761
    TAC = TACE+TACWl+TACWs
    TAC_tablei2$TAC=TAC_tablei$TAC
  }
  ...
  #F25 = 100% of F14 and F26 1.2% of F22
  if (err==0){
    TAC_tablei2$TAC[TAC_tablei2$Fleet %in% c(24:26)] = TAC_tablei$TAC[TAC_tablei$Fleet %in% c(24:26)]
  } else {
    tempc=TAC_tablei2 %>% filter(Fleet %in% c(1:13,15:19))
    tempcs= tempc %>% group_by(Seas) %>% summarize(TACl=sum(TAC))
    TAC_tablei2$TAC[TAC_tablei2$Fleet %in% c(24)]=0.05*tempcs$TACl
  ...
  TAC_dat = list(TAC=TAC, TACWs=TACWs, TACWl=TACWl, TACE=TACE, Discards=Discard, Ftarget=fmult, Fmultiplier=NA, TAC_flt = TAC_tablei2)
    ...
    TAC_dat = list(TAC=17806.76, TACWs=4725, TACWl=7609, TACE=5472.761, Discards=Discard, Ftarget=fmult, Fmultiplier=NA, TAC_flt = TAC_table)
